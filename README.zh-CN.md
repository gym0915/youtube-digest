...
- 把零碎字幕变成清晰、可搜索的学习资料。
- 将 Transcript、Overview 和 Notes 查看为原文、简体中文或双语内容。
...
> 请把这个项目下载或克隆到我选择的长期保留文件夹，告诉我准确的完整路径，并让 Chrome“加载已解压的扩展程序”使用同一个文件夹。如果我在第一次安装时需要位置建议，可以推荐 macOS 或 Linux 上的 `~/Documents/youtube-digest`，或 Windows 上的 `%USERPROFILE%\Documents\youtube-digest`，但不要假设我一定使用这些路径。请用简单易懂的语言一步一步指导我完成安装和配置。https://github.com/gym0915/youtube-digest
...
1. 打开 [github.com/gym0915/youtube-digest](https://github.com/gym0915/youtube-digest)。
...
翻译是延迟按需和渐进式的。只有当前 Transcript、Overview 或 Notes 内容表面的视口及其上下小范围的 120px 预取区内的内容项才会获得新的调用资格；用户滚走后尚未发出的队列项会丢弃，也不会在后台把剩余内容翻完。中文和双语模式会复用已缓存的译文。重试、服务商行为和价格变化都可能增加最终成本。
...
3. 翻译或讲解等功能只发送当前需要的内容，例如选中的文本和上下文，或少量 Transcript、Overview、Notes 内容项。
